...
> **Nota sobre PyTorch:** o torch é resolvido diretamente via PyPI sem índice customizado,
> garantindo compatibilidade com macOS Intel (`x86_64`), macOS Apple Silicon (`arm64`) e Linux.
...
Implementação com `try/except ImportError` para que o módulo continue funcional mesmo
antes de `torch` ou `numpy` estarem instalados.
...
| **Torch via PyPI (sem índice customizado)** | Compatibilidade macOS x86_64 e arm64 sem config extra |
| **BERTopic na deps de prod** | Consumido no stage `feature_eng` do DVC (Etapa 3); não é só dev |
...
| # | Commit | Conteúdo |
|---|--------|----------|
| 1 | `build(deps)` | `pyproject.toml` — todas as deps de prod e dev |
| 2 | `build(lock)` | `uv.lock` — 232 pacotes resolvidos |
| 3 | `chore(env)` | `.env.example` — seções organizadas por etapa |
| 4 | `feat(config)` | `configs/settings.py` — MLflow, DVC, seeds, model/eval |
| 5 | `feat(seeds)` | `src/training/seeds.py` — NumPy + PyTorch seeds |
| 6 | `feat(scripts)` | `scripts/validate_env.py` — diagnóstico de ambiente |
| 7 | `docs` | `docs/DOCUMENTACAO_ETAPA2.md` — este documento |
...
| Arquivo | Ação |
|---------|------|
| `pyproject.toml` | Atualizado — deps prod + dev + extras |
| `uv.lock` | Criado — lock file com 232 pacotes resolvidos |
